...
- **Propósito**: E-commerce + showcase para tienda de ropa masculina
- **Cliente**: Lukess Home (3 puestos en Mercado Mutualista, Santa Cruz, Bolivia)
- **Integración**: Conectado con Lukess Inventory System (Supabase)

no crees ni modifiques ningun archivo de auditoria ni estado actual del proyecto ni nada informativo de lo que hisite yo cuando avancemos el tiempo indicado yo te pedire personalmente crea un archivo .md , por ejemplo AUDIT_09_02_9PM_ecommerce_completo.md donde actualices y pongas al dia de todo lo que hisite y nuevo que implementaste desde la ultima auditoria que hiciste (hay tu te fija cual fue la ultima que hisite para hacer una actualizada con todo lo nuevo)

## 🔗 INTEGRACIÓN CON SISTEMA DE INVENTARIO (CRÍTICO)
...
🛒 CARRITO DE COMPRAS (FUNCIONALIDAD CRÍTICA)
Context del Carrito
typescript
...
💳 PROCESO DE PAGO CON QR (YOLO PAGO)
Modal de Checkout
typescript
...
├── producto/
│   └── [id]/
│       └── page.tsx          # Detalle de producto individual
└── api/
    └── webhook-yolo/
        └── route.ts          # Webhook para confirmar pago (futuro)
...
├── cart/
│   ├── CartButton.tsx       # Ícono con badge de cantidad
│   ├── CartDrawer.tsx       # Drawer lateral con items
│   └── CheckoutModal.tsx    # Proceso de pago
└── producto/
...
✅ SIEMPRE HACER:
tsx
// Gradiente con text-white explícito
...
sql
-- Órdenes de clientes (compras online)
CREATE TABLE orders (
...
Causa: No estás usando Supabase Realtime
Solución:
...
Solución: Siempre hacer SELECT antes de UPDATE
...
🎯 Checklist Pre-Deploy
 ...
 IDs de navegación consistentes
...
Dirección: Mercado Mutualista, Santa Cruz, Bolivia

Horarios: Lun-Sáb 8AM-10PM, Dom 9AM-9PM

⚠️ RECORDATORIO: Este archivo debe referenciarse al inicio de CADA prompt a Cursor con @.cursorrules.md
